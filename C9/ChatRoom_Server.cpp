...
int main(int argc,char*argv[])
{
     ...
     * 并且socket的值可以直接用来索引(作为数组的 下标)socket连接对应的client_data对象，这是将socket和客户数据关联的简单而高效的方式*/
    client_data*users=new client_data[FD_LIMIT];
/*尽管我们分配了足够多的client_data对象，但为了提高poll的性能，仍然有必要 限制用户的数量*/
    pollfd fds[USER_LIMIT+1];
    int user_counter=0;
    for(int i=1;i<=USER_LIMIT;++i) {
        /* 将 fds 数组中索引为 i 的文件描述符设置为无效值 -1，表示未被使用。
         * 将 fds 数组中索引为 i 的事件类型设置为 0，表示不关心任何事件。*/
        fds[i].fd=-1;
        fds[i].events=0;
    }
    fds[0].fd=listenfd;
    ...
    while(1)
    {
//      -1 表示阻塞等待，即 poll() 函数将一直阻塞直到有事件发生为止。
...
/*如果请求太多，则关闭新到的连接*/
            ...
            else if(fds[i].revents&POLLRDHUP)
            {
                 ...
                 * users[] 数组用于记录每个文件描述符对应的用户信息，fds[i].fd 表示当前处理的文件描述符（客户端连接），
                 * 而 fds[user_counter].fd 表示最后一个活跃的客户端连接。当客户端关闭连接时，为了维护 users[] 数组的连续性，
                 * 将最后一个活跃的客户端连接的信息复制到当前处理的文件描述符的位置，然后将用户总数减1，即删除了一个已关闭连接的用户信息。*/
            ...
            {
                int connfd=fds[i].fd; memset(users[connfd].buf,'\0',BUFFER_SIZE);
