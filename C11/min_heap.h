...
/*初始化堆数组*/
            ...
            {
                /*对数组中的第[(cur_size-1)/2]~0个元素执行下虑操作*/ percolate_down(i);
            }
        }
    }
/*销毁时间堆*/
...
private: /*最小堆的下虑操作，它确保堆数组中以第hole个节点作为根的子树拥有最小堆性质*/
...
/*将堆数组容量扩大1倍*/
    void resize()
    {
        heap_timer**temp=new heap_timer*[2*capacity]; for(int i=0;i<2*capacity;++i)
        {
            temp[i]=NULL;
        }
        if(!temp)
